...
def read_dimensions_csv(csv_path: str) -> dict:
    ...
    if not os.path.isfile(csv_path):
        print("WARNING: mgs3_mc_dimensions.csv not found. Skipping verification.\n")
        return entries
        ...
        for row in reader:
            if len(row) >= 3:
                ...
                try:
                    ...
                    continue
    print(f"Loaded {len(entries)} entries from mgs3_mc_dimensions.csv\n")
    return entries

def main():
    # --- Fixed working directory ---
    base_dir = r"C:\Development\Git\MGS3-PS2-Textures\u - dumped from subsistence\dump\Final Rebuilt"

    # CSV is located in the same folder as the script
    script_dir = os.path.dirname(os.path.abspath(__file__))
    dimensions_csv = os.path.join(script_dir, "mgs3_mc_dimensions.csv")
    ...
    for root, dirs, files in os.walk(base_dir):
        ...
        for f in files:
            ...
            if name_no_tga not in csv_data:
                unmatched_count += 1
                log_lines.append(f"[UNMATCHED] {f} | not found in mgs3_mc_dimensions.csv")
                continue
    ...
    print(f"\nMatched in mgs3_mc_dimensions.csv : {matched_count}")
