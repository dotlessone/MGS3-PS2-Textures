...
def main() -> None:
    ...
    sol_root = repo_root / "t - dumped from snake eater" / "dump" / "Final Rebuilt"
    subs_root = repo_root / "u - dumped from subsistence" / "dump" / "Final Rebuilt"
    csv_out = repo_root / "u - dumped from subsistence" / "mgs3_ps2_subsistence_version_dates.csv"
    ...
    with csv_out.open("w", newline="", encoding="utf-8") as f:
        ...
        for stem_lower, subs_hash in tqdm(items, desc="Writing CSV", unit="file"):
            ...
            if sol_hash is not None and sol_hash == subs_hash:
                origin_version = "snake eater"
                origin_date = sol_unix
            else:
                origin_version = "subsistence"
