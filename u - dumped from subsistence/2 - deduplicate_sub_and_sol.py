...
# === CONFIG ===
SUBSTANCE_DIR = Path(r"C:\Development\Git\MGS3-PS2-Textures\u - dumped from subsistence")
SOL_DIR = Path(r"C:\Development\Git\MGS3-PS2-Textures\t - dumped from snake eater")
...
def main():
    ...
    def process_path(rel_path):
        sub_path = subsistence_files.get(rel_path)
        ...
        if sol_path and not sub_path:
            return ("missing_subsistence", rel_path)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            ...
            elif kind == "missing_subsistence":
                missing_in_subsistence.append(str(data))
    sys.stdout.write("\n")
