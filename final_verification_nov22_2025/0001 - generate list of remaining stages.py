...
def main():
    ...
    csv_dir = os.path.join(repo_root, "u - dumped from subsistence")
    texture_map_csv = os.path.join(csv_dir, "mgs3_texture_map.csv")
    ps2_dim_csv = os.path.join(csv_dir, "mgs3_ps2_dimensions.csv")
    ...
    log(f"  - {script_dir}")
    log(f"  - {os.path.join(repo_root, 'u - dumped from subsistence', 'dump', 'Final Rebuilt', 'half_alpha')}")
    log(f"  - {os.path.join(repo_root, 'u - dumped from subsistence', 'dump', 'Final Rebuilt', 'opaque')}")
    log("")
    ...
    # SCAN DIRECTORIES
    # ==========================================================
    final_rebuilt_dir = os.path.join(repo_root, "u - dumped from subsistence", "dump", "Final Rebuilt")
    ...
    for stage, missing in missing_per_stage.items():
        ...
        header_line = f"mgs3/base|mgs3-ps2-text /<{joined_names}>.png\n"
